...
import {
  getHuanxingServerListApi,
  createHuanxingServerApi,
  updateHuanxingServerApi,
  deleteHuanxingServerApi,
} from '#/api/huanxing/server';
import { querySchema, useColumns, formSchema } from './data';

defineOptions({ name: 'HuanxingServer' });
...
const gridOptions: VxeTableGridOptions<HuanxingServerResult> = {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        return await getHuanxingServerListApi({
...
function onActionClick({ code, row }: OnActionClickParams<HuanxingServerResult>) {
  switch (code) {
    case 'delete': {
      deleteHuanxingServerApi([row.id]).then(() => {
...
const [editModal, editModalApi] = useVbenModal({
  ...
  async onConfirm() {
    ...
    if (valid) {
      ...
      try {
        await updateHuanxingServerApi(editId.value, data);
  ...
  onOpenChange(isOpen: boolean) {
    if (isOpen) {
      const data = editModalApi.getData<HuanxingServerResult>();
...
const [addModal, addModalApi] = useVbenModal({
  ...
  async onConfirm() {
    ...
    if (valid) {
      ...
      try {
        await createHuanxingServerApi(data);
        message.success($t('ui.actionMessage.operationSuccess'));
...
<template>
  <Page auto-content-height>
    ...
    <editModal title="编辑服务器" :fullscreen-button="false" class="w-[800px]">
      <EditForm />
    </editModal>
    <addModal title="添加服务器" :fullscreen-button="false" class="w-[800px]">
      <AddForm />
    </addModal>
  </Page>
</template>
