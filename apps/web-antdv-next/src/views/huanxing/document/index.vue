...
} from '#/api/huanxing/document';
import { querySchema, useColumns, formSchema } from './data';

defineOptions({ name: 'HuanxingDocument' });
...
const gridOptions: VxeTableGridOptions<HuanxingDocumentResult> = {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        return await getHuanxingDocumentListApi({
...
function onActionClick({ code, row }: OnActionClickParams<HuanxingDocumentResult>) {
  switch (code) {
    case 'delete': {
      deleteHuanxingDocumentApi(row.id).then(() => {
...
const [editModal, editModalApi] = useVbenModal({
  ...
  async onConfirm() {
    ...
    if (valid) {
      ...
      try {
        await updateHuanxingDocumentApi(editId.value, data);
  ...
  onOpenChange(isOpen: boolean) {
    if (isOpen) {
      const data = editModalApi.getData<HuanxingDocumentResult>();
...
const [addModal, addModalApi] = useVbenModal({
  ...
  async onConfirm() {
    ...
    if (valid) {
      ...
      try {
        await createHuanxingDocumentApi(data);
        message.success($t('ui.actionMessage.operationSuccess'));
...
<template>
  <Page auto-content-height>
    ...
    <editModal title="编辑文档" :fullscreen-button="false" class="w-[800px]">
      <EditForm />
    </editModal>
    <addModal title="添加文档" :fullscreen-button="false" class="w-[800px]">
      <AddForm />
    </addModal>
  </Page>
</template>
