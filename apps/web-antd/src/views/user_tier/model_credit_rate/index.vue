...
import {
  getModelCreditRateListApi,
  createModelCreditRateApi,
  updateModelCreditRateApi,
  deleteModelCreditRateApi,
} from '#/api/user_tier/model_credit_rate';
...
const gridOptions: VxeTableGridOptions<ModelCreditRate> = {
  rowConfig: {
    keyField: 'id',
  },
  checkboxConfig: {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        return await getModelCreditRateListApi({
...
function onActionClick({ code, row }: OnActionClickParams<ModelCreditRate>) {
  switch (code) {
    case 'delete': {
...
const [editModal, editModalApi] = useVbenModal({
  ...
  onOpenChange(isOpen: boolean) {
    if (isOpen) {
      const data = editModalApi.getData<ModelCreditRate>();
